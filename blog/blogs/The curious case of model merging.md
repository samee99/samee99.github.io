...
- [TIES](https://arxiv.org/pdf/2306.01708.pdf) stands for Trim, Elect Sign & Merge: As opposed to SLERP, we can use TIES to combine as many LLMs as want and not just two. In TIES, we first zero all weight values in the base LLMs that are below a certain threshold (trimming).  Then we 
