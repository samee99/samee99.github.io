...
- [TIES](https://arxiv.org/pdf/2306.01708.pdf) stands for Trim, Elect Sign & Merge: As opposed to SLERP, we can use TIES to combine as many LLMs as want and not just two. In TIES, we This involves first zeroing weight values that are below a certain threshold (trimming) in the base LLMs and then averaging all positive weights separately and all negative weights with negative separately. 
