...
Notice that Neo depended on AutoML capability to optimize the retriever and generation tool. To handle more usecases, Neo needs to be able to identify each additional tool it requires and build a mechanism to optimize that tool. Imagine that a user, who is a warehouse manager, requests Neo to build a system that identifies the number of humans in a video stream and send an email to the manager in case more than 10 humans are observed in the video. Neo, in this case, would need to get requirements on how many video frames should be checked and then apply the best available object detection model t 
