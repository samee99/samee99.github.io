...
The ultimate use case for Neo would be to build a system that replicates and improves Neo itself. This could take the form of AutoML capability in terms of identifying LLMs and finding the right hyperparameters (e.g. number of epochs, learning rate, alpha and rank for LoRa typfor fine-tuning them on specific use cases that users want Neo to handle. For example, the Neo lawyer could be instruction fine-tuned on curated AI 
